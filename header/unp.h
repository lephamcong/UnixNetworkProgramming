...
#include <errno.h>      /*  EINTR, ECONNREFUSED, etc                    */
...
#include <sys/uio.h>    /*  for iovec() and resys/wait.h>   /*  for waitpid()                               */
...
support them today. These two macros really need and ALIGN() macro, but each implementation
...
/*
POSIX requires that and #include of <poll.h> define INFTIM,
but many systems still define it in <sys/stropts.h>. We don't want to include all the
STREAMS stuff if it's not needed, so we just define INFTIM here. This is the standard 
value, but there's no guarentee it is -1
*/
#ifndef INFTIM
...
/*  Define some port number that can be used for out examples   */
...
struct sockaddr_storage {
    ...
    char __ss_pad1[__SS_PAD1SIZE];  /*  padding to align */
    int64_t __ss_align; /*  force desired structure storage alignment */
    char __ss_pad2[__SS_PAD2SIZE];  /*  padding to make structure large enough */
};
#endif
